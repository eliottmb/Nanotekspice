...
nts::C4011::C4011(const std::string &name) : _name(name), nb_in(8), nb_out(4)
{
	for (int i = 0; i < 13; i++)
...
{
	_pins[2] = my_nand(_pins[0], _pins[1]);
	_pins[3] = my_nand(_pins[4], _pins[5]);
	_pins[9] = my_nand(_pins[7], _pins[8]);
	_pins[10] = my_nand(_pins[11], _pins[12]);
	return _pins[pin];
}
...
void		nts::C4011::setState(nts::Tristate state, size_t pinNumber)
{
	_pins[pinNumber] = state;
}

nts::Tristate	nts::C4011::getPinAddr(size_t pinNum)
{
	return _pins[pinNum];
}
